...
namespace Zgine {
	...
	// Additional 2D primitives implementation
	void BatchRenderer2D::DrawTriangle(const glm::vec3& p0, const glm::vec3& p1, const glm::vec3& p2, const glm::vec4& color)
	{
		if (s_QuadIndexCount >= MaxIndices - 3)
			NextBatch();

		float textureIndex = 0.0f; // White Texture

		// Add triangle vertices
		s_QuadVertexBufferPtr->Position = p0;
		s_QuadVertexBufferPtr->Color = color;
		s_QuadVertexBufferPtr->TexCoord = { 0.0f, 0.0f };
		s_QuadVertexBufferPtr->TexIndex = textureIndex;
		s_QuadVertexBufferPtr++;

		s_QuadVertexBufferPtr->Position = p1;
		s_QuadVertexBufferPtr->Color = color;
		s_QuadVertexBufferPtr->TexCoord = { 0.5f, 1.0f };
		s_QuadVertexBufferPtr->TexIndex = textureIndex;
		s_QuadVertexBufferPtr++;

		s_QuadVertexBufferPtr->Position = p2;
		s_QuadVertexBufferPtr->Color = color;
		...
		s_QuadIndexCount += 3;
		s_Stats.QuadCount++; // Count as one primitive
	}

	void BatchRenderer2D::DrawEllipse(const glm::vec3& position, float radiusX, float radiusY, const glm::vec4& color, int segments)
	{
		if (s_QuadIndexCount >= MaxIndices - segments * 3)
			NextBatch();
		...
		s_QuadVertexBufferPtr->Color = color;
		s_QuadVertexBufferPtr->TexCoord = { 0.5f, 0.5f };
		s_QuadVertexBufferPtr->TexIndex = textureIndex;
		s_QuadVertexBufferPtr++;
		...
		for (int i = 0; i <= segments; i++)
		{
			...
			glm::vec3 vertexPos = position + glm::vec3(cos(angle) * radiusX, sin(angle) * radiusY, 0.0f);
			
			s_QuadVertexBufferPtr->Position = vertexPos;
			s_QuadVertexBufferPtr->Color = color;
			s_QuadVertexBufferPtr->TexCoord = { 0.5f + 0.5f * cos(angle), 0.5f + 0.5f * sin(angle) };
			s_QuadVertexBufferPtr->TexIndex = textureIndex;
			s_QuadVertexBufferPtr++;
		}

		s_QuadIndexCount += segments * 3;
		s_Stats.QuadCount++;
	}

	void BatchRenderer2D::DrawEllipseOutline(const glm::vec3& position, float radiusX, float radiusY, const glm::vec4& color, float thickness, int segments)
	{
		if (s_QuadIndexCount >= MaxIndices - segments * 6)
			NextBatch();
		...
		for (int i = 0; i < segments; i++)
		{
			...
			// Add 4 vertices for the line segment
			s_QuadVertexBufferPtr->Position = p1 - perpendicular;
			s_QuadVertexBufferPtr->Color = color;
			s_QuadVertexBufferPtr->TexCoord = { 0.0f, 0.0f };
			s_QuadVertexBufferPtr->TexIndex = textureIndex;
			s_QuadVertexBufferPtr++;
			...
			s_QuadVertexBufferPtr->Position = p2 + perpendicular;
			s_QuadVertexBufferPtr->Color = color;
	...
	void BatchRenderer2D::DrawQuadGradient(const glm::vec3& position, const glm::vec2& size, const glm::vec4& colorTopLeft, const glm::vec4& colorTopRight, const glm::vec4& colorBottomLeft, const glm::vec4& colorBottomRight)
	{
		...
		s_QuadVertexBufferPtr->Position = position + glm::vec3(-size.x * 0.5f, -size.y * 0.5f, 0.0f);
		s_QuadVertexBufferPtr->Color = colorBottomLeft;
		s_QuadVertexBufferPtr->TexCoord = { 0.0f, 0.0f };
		s_QuadVertexBufferPtr->TexIndex = textureIndex;
		s_QuadVertexBufferPtr++;

		s_QuadVertexBufferPtr->Position = position + glm::vec3(size.x * 0.5f, -size.y * 0.5f, 0.0f);
		s_QuadVertexBufferPtr->Color = colorBottomRight;
		s_QuadVertexBufferPtr->TexCoord = { 1.0f, 0.0f };
		s_QuadVertexBufferPtr->TexIndex = textureIndex;
		s_QuadVertexBufferPtr++;

		s_QuadVertexBufferPtr->Position = position + glm::vec3(size.x * 0.5f, size.y * 0.5f, 0.0f);
		s_QuadVertexBufferPtr->Color = colorTopRight;
		s_QuadVertexBufferPtr->TexCoord = { 1.0f, 1.0f };
		s_QuadVertexBufferPtr->TexIndex = textureIndex;
		s_QuadVertexBufferPtr++;

		s_QuadVertexBufferPtr->Position = position + glm::vec3(-size.x * 0.5f, size.y * 0.5f, 0.0f);
		s_QuadVertexBufferPtr->Color = colorTopLeft;
		s_QuadVertexBufferPtr->TexCoord = { 0.0f, 1.0f };
		s_QuadVertexBufferPtr->TexIndex = textureIndex;
		s_QuadVertexBufferPtr++;
	...
	void BatchRenderer2D::DrawRotatedQuadGradient(const glm::vec3& position, const glm::vec2& size, float rotation, const glm::vec4& colorTopLeft, const glm::vec4& colorTopRight, const glm::vec4& colorBottomLeft, const glm::vec4& colorBottomRight)
	{
		...
		// Add vertices with gradient colors
		s_QuadVertexBufferPtr->Position = transform * glm::vec4(-0.5f, -0.5f, 0.0f, 1.0f);
		s_QuadVertexBufferPtr->Color = colorBottomLeft;
		s_QuadVertexBufferPtr->TexCoord = { 0.0f, 0.0f };
		s_QuadVertexBufferPtr->TexIndex = textureIndex;
		s_QuadVertexBufferPtr++;
