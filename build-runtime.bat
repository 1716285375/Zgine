@echo off
REM ============================================================================
REM Build ZgineRuntime Only (Core Engine Library)
REM ============================================================================
setlocal enabledelayedexpansion

echo.
echo ================================================
echo    Zgine Runtime Builder
echo ================================================
echo.
...
REM ============================================
echo [5/6] Configuring ZgineRuntime with preset: %CMAKE_PRESET% (PCH=%ZGINE_USE_PCH%)...

if exist "build\%CMAKE_PRESET%\CMakeCache.txt" del /f /q "build\%CMAKE_PRESET%\CMakeCache.txt"

cmake --preset=%CMAKE_PRESET% -DZGINE_USE_PCH=%ZGINE_USE_PCH% -DZGINE_BUILD_SANDBOX=OFF -DZGINE_BUILD_EDITOR=OFF -DZGINE_BUILD_TESTS=OFF
if errorlevel 1 (
...
REM Step 6: Build
REM ============================================
echo [6/6] Building ZgineRuntime...
cmake --build --preset=%CMAKE_PRESET% --parallel 8 --target ZgineRuntime
if errorlevel 1 (
    echo Build FAILED!
    pause
    exit /b 1
...
echo ================================================
echo    Library: build\%CMAKE_PRESET%\lib\ZgineRuntime.lib
if "%ZGINE_BUILD_SHARED%"=="ON" echo    (or ZgineRuntime.dll if shared)
echo.
pause
