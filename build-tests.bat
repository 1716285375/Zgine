@echo off
REM ============================================================================
REM Build ZgineRuntime + ZgineTests
REM ============================================================================
setlocal enabledelayedexpansion

echo.
echo ================================================
echo    Zgine Tests Builder
echo ================================================
echo.
...
REM ============================================
echo [5/6] Configuring Zgine (Runtime + Tests) with preset: %CMAKE_PRESET% (PCH=%ZGINE_USE_PCH%)...

if exist "build\%CMAKE_PRESET%\CMakeCache.txt" del /f /q "build\%CMAKE_PRESET%\CMakeCache.txt"

cmake --preset=%CMAKE_PRESET% -DZGINE_USE_PCH=%ZGINE_USE_PCH% -DZGINE_BUILD_SANDBOX=OFF -DZGINE_BUILD_EDITOR=OFF -DZGINE_BUILD_TESTS=ON
if errorlevel 1 (
...
REM Step 6: Build
REM ============================================
echo [6/6] Building ZgineRuntime and ZgineTests...
...
echo ================================================
set "EXE_PATH=build\%CMAKE_PRESET%\bin\ZgineTests.exe"
if exist "build\%CMAKE_PRESET%\bin\Release\ZgineTests.exe" set "EXE_PATH=build\%CMAKE_PRESET%\bin\Release\ZgineTests.exe"
if exist "build\%CMAKE_PRESET%\bin\Debug\ZgineTests.exe" set "EXE_PATH=build\%CMAKE_PRESET%\bin\Debug\ZgineTests.exe"

if exist "%EXE_PATH%" (
    echo    Test executable: "%EXE_PATH%"
    set /p RUN="Run tests now? (y/n): "
    if /i "!RUN!"=="y" (
    ...
    echo    WARNING: Test executable not found at expected location
)
echo.
pause
